...
static void pushQue(pcb_t *ptr);
static pcb_t* popQue(void);
/*
...
static pcb_t* popQue(){
   ...
   if(algorithm=='1'){
        ...
        if(head==NULL){node=NULL;}
            ...
            while (current != NULL) {
                if (time <current->time_remaining){
                    time = current->time_remaining;}
                current = current->next;}
            previous = head;
            current = head;
            while(current !=NULL){
                if (time == current->time_remaining){node = current;
                    if (current != head){previous->next = current->next;} 
...
extern void wake_up(pcb_t *process)
{
    ...
    if (algorithm=='1'){
        pthread_mutex_lock(&current_mutex);
        int lower_remaining_time=0;
        int idle =-1;
        if(idle!=-1 && lower_remaining_time<process->time_remaining){
            force_preempt(idle);
        }
        for (int i=0;i<cpuCount;i++){
            ...
            else if(lower_remaining_time>current[i]->time_remaining){
                idle=i;
                lower_remaining_time = current[i]->time_remaining;
            }
        }
        pthread_mutex_unlock(&current_mutex);
    }
